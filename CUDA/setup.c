...
/**
 * @brief Allocate memory on the GPU for the main arrays.
 */
static void allocate_device_arrays() {
 ...
 */
static void free_device_arrays() {
    cudaFree(d_u);
    cudaFree(d_v);
    cudaFree(d_p);
    cudaFree(d_b);
...
/**
 * @brief Allocate all of the arrays used for computation (both CPU and GPU) (double precision).
 */
...
/**
 * @brief Free all of the arrays used for the computation (both CPU and GPU)
 */
void free_arrays() {
    free_2d_array(u);
    free_2d_array(v);
